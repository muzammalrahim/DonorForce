...
class com_donorforceInstallerScript
{
	...
	function postflight($type, $parent) 
	{
		...
		if(!isset($columns7['pobox'])){ 
   			$alterQuery36 = "ALTER TABLE `#__donorforce_invoice_temp` ADD `pobox` varchar(255) NOT NULL";
			$db->setQuery($alterQuery36 );
			$db->execute();  			
		}
		if(!isset($columns7['org_name'])){
			$alterQuery37 = "ALTER TABLE `#__donorforce_invoice_temp` ADD `org_name` varchar(300) AFTER `id`"; 
		 	$db->setQuery($alterQuery37);
		 	$db->execute();
		}
		 
		if(!isset($columns['menbership_type'])){ 
			$alterQuery38 = "ALTER TABLE `#__donorforce_donor` ADD `menbership_type` varchar(100)"; 
