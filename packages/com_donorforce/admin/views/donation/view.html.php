...
class DonorforceViewDonation extends JViewLegacy
{
	protected $form;
	protected $item;
	protected $state;
	...
	protected function addToolbar()
	{
		...
		$isNew		= ($this->item->donation_id == 0);
		$user	= JFactory::getUser();
		JToolBarHelper::title(JText::_('Donation Edit'), 'newsfeeds.png');
		$canDo		= $this->getActions($this->state->get('filter.id'), $this->item->donation_id);
		...
		//New button is added for the function of save&new
	...
	public static function getActions($classId = 0)
	{
		...
		if (empty($id)) {
			...
			$assetName = 'com_donorforce.donations.'.(int) $id;
		}
