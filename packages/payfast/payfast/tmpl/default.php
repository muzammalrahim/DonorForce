...
/*echo "<pre>"; 
print_r($vars);*/
$return_url=  JUri::root().'index.php?option=com_donorforce&view=orders&layout=order&processor=payfast&orderid='.$vars->order_id.'&taskaction=return';
if(isset($vars->return) && !empty($vars->return)){
	$return_url=  htmlspecialchars_decode($vars->return).'&taskaction=return';
}
//;//$vars->return; //str_replace('&amp;','&',$vars->return);
$cancel_url=htmlspecialchars_decode($vars->cancel_return);
$notify_url= htmlspecialchars_decode($vars->notify_url);
