 ...
 * */
	...
	{ 
		...
		file_put_contents(dirname(__FILE__).'/payfast.txt','---------------isvalidipn----------------', FILE_APPEND );		
		file_put_contents(dirname(__FILE__).'/payfast.txt', print_r($data, true), FILE_APPEND );
		file_put_contents(dirname(__FILE__).'/payfast.txt','returnString='.$returnString, FILE_APPEND );		
		file_put_contents(dirname(__FILE__).'/payfast.txt','md5='.md5($returnString), FILE_APPEND );	 
	file_put_contents(dirname(__FILE__).'/payfast.txt','--/--', FILE_APPEND );	
		...
		{	
				...
				CURLOPT_POST => true,
				CURLOPT_POSTFIELDS => $returnString,
				);
				...
				if( $res === false )
				{
					...
					// echo "<pre> curlOpts = "; print_r($curlOpts); echo "</pre>"; 				
				}else{
						return true;	
				}
